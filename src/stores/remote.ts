  ...
  constructor(
    ...
    if (opts.auth?.basic) {
      this.headers["Authorization"] =
        `Basic ${btoa(`${opts.auth.basic.username}:${opts.auth.basic.password}`)}`;
    }
  }
          ...
          switch (r.status) {
            case 400:
              throw new ResonateError(
                "Invalid request",
                ErrorCodes.STORE_PAYLOAD,
                body,
              );
            case 401:
              throw new ResonateError(
                "Unauthorized request",
                ErrorCodes.STORE_UNAUTHORIZED,
                body,
              );
            case 403:
              throw new ResonateError(
...
export class RemotePromiseStore implements IPromiseStore {
  ...
  async cancel(
    ...
    const promise = await this.store.call(
      `promises/${id}`,
      isDurablePromiseRecord,
      {
        method: "PATCH",
    ...
    const schedule = this.store.call<Schedule>(`schedules`, isSchedule, {
      ...
      body: JSON.stringify({
        ...
        promiseParam: {
          ...
          data: promiseData
            ? encode(promiseData, this.store.encoder)
            : undefined,
        },
        promiseTags,
      }),
...
function decode<P extends DurablePromiseRecord>(
  promise: P,
