...
//
// The group decides which poll stream a worker listens on. Resolving it from
// the environment lets a deployment place a worker in a group the application
// was never written to name — the Resonate server's `modal://` transport
// assigns each sandbox its own group and passes it as RESONATE_GROUP.
// ---------------------------------------------------------------------------

describe("worker group resolution", () => {
  const original = process.env.RESONATE_GROUP;

  afterEach(() => {
    if (original === undefined) {
      delete process.env.RESONATE_GROUP;
    } else {
      process.env.RESONATE_GROUP = original;
    }
  });

  function groupOf(resonate: Resonate): string {
    // local://any@<group>/<pid>
    const anycast: string = (resonate as any).network.anycast;
    return anycast.split("@")[1].split("/")[0];
  }

  test("defaults to 'default'", async () => {
    delete process.env.RESONATE_GROUP;
    const resonate = new Resonate({ logLevel: "error" });
    expect(groupOf(resonate)).toBe("default");
    await resonate.stop();
  });

  test("falls back to RESONATE_GROUP", async () => {
    process.env.RESONATE_GROUP = "sandbox-7";
    const resonate = new Resonate({ logLevel: "error" });
    expect(groupOf(resonate)).toBe("sandbox-7");
    await resonate.stop();
  });

  test("an explicit group wins over the environment", async () => {
    process.env.RESONATE_GROUP = "sandbox-7";
    const resonate = new Resonate({ group: "mine", logLevel: "error" });
    expect(groupOf(resonate)).toBe("mine");
    await resonate.stop();
  });

  test("an empty RESONATE_GROUP is ignored", async () => {
    process.env.RESONATE_GROUP = "";
    const resonate = new Resonate({ logLevel: "error" });
    expect(groupOf(resonate)).toBe("default");
    await resonate.stop();
  });
