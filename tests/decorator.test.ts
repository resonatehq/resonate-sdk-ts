...
import { value, type Yieldable } from "../src/types";
...
describe("Decorator", () => {
  ...
  it("returns final value after multiple yields", () => {
    function* foo(ctx: Context): Generator<LFI<any> | Future<any>, any, number> {
      yield new Future("future-1", "completed", value(10));
    ...
    d.next({ type: "internal.literal", value: value(10) }); // yield a future, get a literal back
    const r = d.next({
      type: "internal.promise",
      state: "completed",
      id: "abc.2",
  ...
  it("awaits if there are pending invokes - Structured Concurrency", () => {
    function* foo(ctx: Context): Generator<Yieldable, any, number> {
      yield new Future("future-1", "completed", value(10)); // A
    ...
    d.next({ type: "internal.nothing" }); // First yield
    d.next({ type: "internal.literal", value: value(10) }); // A -> yield a future, get a literal back
    d.next({
      type: "internal.promise",
      state: "pending",
      mode: "attached",
