 ...
 * 2. `unicast` address appears in register_listener requests (used for direct routing)
 * 3. `anycast` address appears as `resonate:target` in run/rpc promise creation
 * 4. Messages received via `network.recv` trigger `onMessage` processing
 *
 * The only idiom change: `run`/`rpc` return a handle (no `begin*` forms), so
 * values come from `handle.result()`.
 */
...
describe("Resonate <-> Network Wiring", () => {
  ...
  test("injecting a custom Network via the constructor works end-to-end (send + recv)", async () => {
    ...
    resonate.register("greet", async (_info: Info, name: string): Promise<string> => {
      return `Hello, ${name}!`;
    });

    // Perform a run -- this will use our mock network's send()
    const result = await (await resonate.run("test-send-1", "greet", "world")).result();
  ...
  test("unicast address appears in register_listener after task.create conflict", async () => {
    ...
    resonate.register("myFunc", async (_info: Info): Promise<string> => "done");
  ...
  test("anycast address appears as resonate:target in run promise creation", async () => {
    ...
    resonate.register("targetFunc", async (_info: Info): Promise<string> => "targeted");

    await (await resonate.run("anycast-test-1", "targetFunc")).result();
  ...
  test("resonate:target is set in rpc promise creation", async () => {
    const mockNet = new MockNetwork({
    ...
    resonate.register("remoteFunc", async (_info: Info): Promise<string> => "remote-result");

    // rpc uses promiseCreate which sets target from opts.target
    await (await resonate.rpc("rpc-test-1", "remoteFunc")).result();
  ...
  test("messages received via network.recv trigger onMessage processing (unblock)", async () => {
    ...
    resonate.register("waitFunc", async (_info: Info): Promise<string> => "should-not-execute-locally");

    // Start a run -- the promise is pending (and no task was handed back), so
    // result() will block until the server notifies us.
    const handle = await resonate.run("recv-test-1", "waitFunc");
    ...
    expect(await result).toBe("delivered-via-recv");
  ...
  test("network.match is used to resolve a plain string target in rpc", async () => {
    ...
    resonate.register("targetedFunc", async (_info: Info): Promise<string> => "ok");

    await (await resonate.rpc("match-wiring-1", "targetedFunc", resonate.options({ target: "my-workers" }))).result();
  ...
  test("URL targets bypass network.match and are stored verbatim in resonate:target", async () => {
    ...
    resonate.register("urlTargetFunc", async (_info: Info): Promise<string> => "ok");

    const urlTarget = "http://some-remote-worker:9000";
    await (await resonate.rpc("match-url-1", "urlTargetFunc", resonate.options({ target: urlTarget }))).result();
