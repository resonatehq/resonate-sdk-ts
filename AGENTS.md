...
> **Read the README first for the user-facing overview.** This file is the agent orientation: what to know before editing the SDK source. Voice = Echo. Treat the SDK as a product surface — it's the most-used Resonate SDK and ships to npm.
...
A more granular stack table lives in this repo's README; only restate it here when an agent needs the runtime/linter/test trio at a glance.
...
npm run check:fix      # biome lint with --write
...
- **Network layer** lives in `src/network/` — talks HTTP to the Rust server (server protocol version pinned to the SDK's `0.10.x` line). Re-runs are idempotent on the server side; the SDK assumes server linearizability.
