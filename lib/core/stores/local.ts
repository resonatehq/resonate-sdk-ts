...
export class LocalPromiseStore implements IPromiseStore {
  ...
  async create(
    ...
    return this.storage.rmw(id, (promise) => {
      ...
      if (strict && !isPendingPromise(promise)) {
        throw new ResonateLocalStoreError(ErrorCodes.FORBIDDEN, "Forbidden request");
      }
  ...
  async resolve(
    ...
    return this.storage.rmw(id, (promise) => {
      if (!promise) {
        throw new ResonateLocalStoreError(ErrorCodes.NOT_FOUND, "Not found");
      }
    ...
    const schedule = await this.storage.rmw(id, (schedule) => {
      if (schedule) {
        if (schedule.idempotencyKey === undefined || ikey != schedule.idempotencyKey) {
          throw new ResonateLocalStoreError(ErrorCodes.ALREADY_EXISTS, "Already exists");
        }
        return schedule;
      }
      ...
      try {
        ...
        throw ResonateLocalStoreError.fromError(error);
      }
